...
module Test = struct
  ...
  let run ?(long = false) ~seed (self : t) : _ result =
    match
      let what = (match self.name with None -> "" | Some s -> s ^ " ") in
      match self.run with
      | T { prop } ->
      ...
      | Q
        ...
        let rand =
          let bits =
            (let l = ref [] in String.iter (fun c -> l := c :: !l) seed; List.rev !l) |> List.map Char.code |> Array.of_list
          in
        ...
        let pp_cex out (cx : _ Q.TestResult.counter_ex) =
          ...
          | None ->
            Format.fprintf out "<instance> (after %d shrink steps)%s" n msg_l
          | Some p ->
            Format.fprintf out "`%s` (after %d shrink steps)%s" (p instance) n
              msg_l
        in
        ...
        | QCheck.TestResult.Failed { instances } ->
          let msg =
            Format.asprintf "@[<v2>%sfailed on instances:@ %a@]" what
              (fun fmt lst -> List.iter (fun x -> pp_cex fmt x; Format.fprintf fmt ";@ ") lst)
              instances
          in
          Error msg
...
let run_all ?seed:seed_hex ?(long = long) ~descr (l : Test.t list list) : unit =
  ...
  let seed_hex =
    ...
    | None, None ->
      Random.self_init ();
      let a = Array.init 8 (fun _ -> Random.int 256 |> Char.chr) |> Array.to_list in
      let hex = String.concat "" (List.map (Printf.sprintf "%02x") (List.map Char.code a)) in
      hex
  in
  ...
  let seed =
    let s = ref "" in
    (try
      for i = 0 to String.length seed_hex / 2 - 1 do
        let byte = String.sub seed_hex (i * 2) 2 in
        let c = Char.chr (int_of_string ("0x" ^ byte)) in
        s := !s ^ String.make 1 c
      done;
      !s
    with _ ->
